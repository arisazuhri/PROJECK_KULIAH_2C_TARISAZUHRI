...
if(!empty($_POST['input_barang_validate'])){
    $select = mysqli_query($conn, "SELECT * FROM tb_daftar_barang WHERE namabarang = '$namabarang'");
    if(mysqli_num_rows($select) > 0){
        $message = '<script>alert("barang yang dimasukkan telah ada");
        window.location="../daftarbarang"</script>';
    }else{
    $query = mysqli_query($conn, "UPDATE tb_daftar_barang SET namabarang='$name',id_barang='$id_barang', biaya='$biaya', tglservis='$tglservis',tglselesai_servis='$tglselesai_servis' WHERE id_barang='$id_barang'");
    if($query){
