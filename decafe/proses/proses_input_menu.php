<?php 
include "connect.php";
$name_menu = (isset($_POST['name_menu'])) ? htmlentities($_POST['name_menu']) : "";
...
if($statusUpload == 0){
    $message = '<script>alert("'.$messageGambar.', Gambar tidak dapat diupload");
    ...
    if(mysqli_num_rows($select) > 0) {
        ...
        if(move_uploaded_file($_FILES['foto']['tmp_name'],$target_file)){
            $query = mysqli_query($conn, "INSERT INTO tb_daftar_menu (foto, nama_menu,keterangan,kategori,harga,stok) values ('".$kode_rand.$_FILES['foto']['name']."','$name_menu','$keterangan','$kat_menu','$harga', $stok')");
            if($query) {
